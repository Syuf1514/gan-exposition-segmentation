...
class GlobalEMMaskGenerator(torch.nn.Module):
    def __init__(self, em_steps):
        super().__init__()
        self.em_steps = em_steps
        self.eps = torch.finfo(torch.float32).eps
    ...
    def m_step(self, padded_images, shifted_images, generated_probs, n_classes, batch_size, rgb_channels, device):
        ops_first_part = torch.einsum('uaij, ukij, ubij -> kab', shifted_images, generated_probs, padded_images)
        ops_second_part = torch.einsum('uaij, ukij, ubij -> kab', padded_images, generated_probs, padded_images)
        ops_norm_factor = torch.einsum('kaa -> k', ops_second_part).reshape(n_classes, 1, 1)
        ops_first_part_normed = ops_first_part / ops_norm_factor
        ops_second_part_normed = ops_second_part / ops_norm_factor + \
                                 self.eps * torch.eye(rgb_channels + 1, device=device).repeat(n_classes, 1, 1)
        ops = torch.einsum('kab, kbc -> kac', ops_first_part_normed, ops_second_part_normed.inverse())

        probs_norm_factor = generated_probs.sum(dim=(0, 2, 3)).reshape(1, n_classes, 1, 1)
        predicted_probs_normed = generated_probs / probs_norm_factor
        difference = shifted_images.repeat(n_classes, 1, 1, 1, 1) - \
                     torch.einsum('kab, ubij -> kuaij', ops, padded_images)
        sigmas = torch.einsum('kuaij, ukij, kubij -> kab', difference, predicted_probs_normed, difference) + \
                 self.eps * torch.eye(rgb_channels, device=device).repeat(n_classes, 1, 1)
    ...
    def e_step(self, padded_images, shifted_images, ops, sigmas, n_classes, image_shape, batch_size):
        difference = shifted_images.repeat(n_classes, 1, 1, 1, 1) - \
                     torch.einsum('kab, ubij -> kuaij', ops, padded_images)
        log_masks = -0.5 * torch.einsum('kuaij, kab, kubij -> ukij', difference, sigmas.inverse(), difference) + \
                    -0.5 * torch.logdet(sigmas).reshape(1, n_classes, 1, 1)
        return log_masks
...
class TrivialMaskGenerator(torch.nn.Module):
    def __init__(self):
        super().__init__()
        self.eps = torch.finfo(torch.float32).eps
    ...
    def forward(self, batch):
        ...
        padded_images = torch.cat((images, torch.ones(batch_size, 1, *image_shape, device=device)), dim=1)
        generated_probs = torch.ones(batch_size, 1, *image_shape, device=device)
        ops, sigmas = self.m_step(padded_images, shifted_images, generated_probs,
                                  n_classes, batch_size, rgb_channels, device)
        generated_masks = self.e_step(padded_images, shifted_images, ops, sigmas,
                                      n_classes, image_shape, batch_size)
